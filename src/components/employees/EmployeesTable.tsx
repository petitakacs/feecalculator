...
export function EmployeesTable({
  ...
  const handleDrop = async (targetId: string) => {
    ...
    const fromIdx = reordered.findIndex((e) => e.id === dragId);
    const toIdx = reordered.findIndex((e) => e.id === targetId);
    ...
    try {
      ...
      showToast("Sorrend mentése sikertelen", "error");
    }
  };
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-200">
            {visible.map((emp) => {
              const isDragOver = dragOverId === emp.id && dragId !== emp.id;
              return (
                <tr
                  key={emp.id}
                  draggable={canWrite}
                  onDragStart={() => setDragId(emp.id)}
                  onDragOver={(e) => { e.preventDefault(); setDragOverId(emp.id); }}
                  onDrop={() => handleDrop(emp.id)}
                  onDragEnd={() => { setDragId(null); setDragOverId(null); }}
                  className={`${emp.active ? "hover:bg-gray-50" : "bg-gray-50 opacity-60"} ${isDragOver ? "bg-indigo-50 outline outline-2 -outline-offset-2 outline-indigo-400" : ""}`}
                >
                  {canWrite && (
                    <td className="px-2 py-4 text-center cursor-grab active:cursor-grabbing text-gray-300 hover:text-gray-500">
            ...
            {visible.length === 0 && (
              <tr>
                <td colSpan={canWrite ? 8 : 7} className="px-6 py-8 text-center text-gray-500">
                  {selectedLocationId
